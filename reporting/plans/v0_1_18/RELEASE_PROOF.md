...
|---|---|---|---|
| **W-OB-1** | README quickstart pivot to `hai init --guided` (mostly pre-staged); cross-reference sweep | **closed** | README §"Install and quickstart" already shows `hai init --guided` as the recommended interactive command (lines 214-226 at HEAD `9c651da`). `agent_integration.md:27` install-lead summary updated to mention `--guided`. Historical launch material under `reporting/docs/launch/` (v0.1.0-era drafts) intentionally left as-is per AGENTS.md provenance discipline. CHANGELOG v0.1.18 entry names W-OB-1. Pure docs; no test change. |
...
| **W-OB-3** | `--guided` prompt content review — content-only `next_action_hint` + skip-input affordance tests | **closed** | All 6 acceptance items pass (PLAN §2.C 1-6). New `next_action_hint` field on `OnboardingResult`, populated based on auth + intent status (`hai daily` direct / `hai auth intervals-icu` remediation / `hai intent` remediation / re-run `hai init --guided`). Empty-input affordance tests cover focus prompt + per-target prompts (no literal `skip` keyword added — per F-PLAN-06 R1 disposition). 6 new tests in `test_init_onboarding_w_ob_3.py`; 9 existing tests in `test_init_onboarding_flow.py` unchanged + green. Informed by W-OB-4a F-OB-4A-02 finding. |
...
| **W-OB-5** | `hai doctor` `next_action` across hint-emitting checks; manifest-consistency invariant | **closed** | All 6 acceptance items pass (PLAN §2.E 1-6). New `_NEXT_ACTION_REGISTRY` table in `core/doctor/checks.py` covering 9 commands (hai init, state init, state migrate, auth intervals-icu, auth garmin, setup-skills, intent training add-session, target set, pull --source intervals_icu). 5 doctor checks gain `next_action` field on WARN/FAIL: `check_state_db`, `check_onboarding_readiness`, `check_auth_intervals_icu`, `check_auth_garmin`, `check_skills`. PASS results omit `next_action`. Manifest-consistency test caught + corrected 3 pre-impl drift entries (state migrate / intent training add-session / target set were registry-False but manifest-True; corrected). F-OB-4A-02 umbrella preference enforced: multi-missing preconditions → `hai init`; single missing → per-component. Renderer adds `next:` line per check after `hint`. **Runtime-only per OQ-4** — no `hai capabilities --json` schema delta; v0.2.3 W-30 freeze remains untouched. |
