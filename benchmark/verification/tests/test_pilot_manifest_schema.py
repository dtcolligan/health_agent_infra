...
def test_locked_manifest_sources_all_20_embedded_input_hashes_from_sidecar(
    tmp_path: Path,
) -> None:
    ...
    assert manifest["locked_hashes"]["total_count"] == 20
    assert len(manifest["locked_hashes"]["fixed_files"]) == 4
    assert len(manifest["locked_hashes"]["task_files"]) == 16
