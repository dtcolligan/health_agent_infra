...
frame, read [`../../project/FRAME.md`](../../project/FRAME.md) and
[`../../project/DECISIONS.md`](../../project/DECISIONS.md), then
...
``SKILL.md``, it's judgment; if it's under ``hai/verification/tests/``, it
locks an invariant; if it's under ``hai/verification/evals/``, it scores
...
- [`../../hai/reporting/plans/tactical_plan_v0_1_x.md`](../../hai/reporting/plans/tactical_plan_v0_1_x.md)
  — HAI runtime backlog and release-history plan. Repo-wide research
  priority lives in [`../../project/FRAME.md`](../../project/FRAME.md)
...
Six domains, each in ``hai/src/health_agent_infra/domains/<d>/``:
...
Core orchestration in ``hai/src/health_agent_infra/core/``:
...
- ``expert-explainer/`` — read-only explanation over allowlisted local
  sources.
- ``review-protocol/``, ``hai/reporting/``, ``safety/`` — cross-
  cutting.
...
``hai/reporting/artifacts/archive/`` and no longer reflect the runtime.

The eval framework is **packaged inside the wheel** at
``hai/src/health_agent_infra/evals/``: ``scenarios/<d>/*.json`` are the
...
``hai/verification/evals/``.

Deterministic runtime coverage is full. Skill coverage is partial and lives
outside packaged ``hai eval``: ``hai/verification/evals/skill_harness/`` covers
recovery + running readiness replay/live paths, and
``hai/verification/evals/synthesis_harness/`` scores synthesis-skill output
fixtures. See ``hai/verification/evals/skill_harness_blocker.md`` for the
...
- No complete skill-narration eval harness across all domains or live CI.
  ...
  scorer under ``hai/verification/evals/``.
...
| "Is it tested?" | ``hai/verification/tests/`` plus the latest release gate in [current_system_state.md](current_system_state.md) |
| "Does it have evals?" | ``hai/verification/evals/`` + ``hai eval run --domain <d>`` |
| "Does it run on live wearable data?" | ``hai auth intervals-icu`` then ``hai pull --source intervals_icu`` |
| "How did we get here?" | [hai/reporting/AUDIT.md](../../hai/reporting/AUDIT.md) + release folders under ``hai/reporting/plans/`` |
