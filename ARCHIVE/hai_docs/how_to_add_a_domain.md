...
```
hai/src/health_agent_infra/domains/<d>/
...
```
hai/src/health_agent_infra/skills/<d>-readiness/SKILL.md
```
...
[`domains/<d>/schemas.py`](../../hai/src/health_agent_infra/domains/running/schemas.py)
...
- **`<Domain>Proposal`** — the shape the skill emits and `hai
  ...
  [`core/schemas.py`](../../hai/src/health_agent_infra/core/schemas.py).
...
[`core/writeback/proposal.py :: DOMAIN_ACTION_ENUMS`](../../hai/src/health_agent_infra/core/writeback/proposal.py)
for prior art.

### 2. Classify

[`domains/<d>/classify.py`](../../hai/src/health_agent_infra/domains/running/classify.py)
...
[`core/config.py :: DEFAULT_THRESHOLDS`](../../hai/src/health_agent_infra/core/config.py)
...
[`domains/<d>/policy.py`](../../hai/src/health_agent_infra/domains/running/policy.py)
...
- [`core/writeback/proposal.py`](../../hai/src/health_agent_infra/core/writeback/proposal.py):
...
- [`core/synthesis.py`](../../hai/src/health_agent_infra/core/synthesis.py):
...
- [`core/synthesis_policy.py`](../../hai/src/health_agent_infra/core/synthesis_policy.py):
...
[`core/state/snapshot.py :: build_snapshot`](../../hai/src/health_agent_infra/core/state/snapshot.py)
...
- Add a migration under
  [`core/state/migrations/`](../../hai/src/health_agent_infra/core/state/migrations/).
...
- Add a projector module under
  [`core/state/projectors/`](../../hai/src/health_agent_infra/core/state/projectors/).
  Model it on
  [`sleep.py`](../../hai/src/health_agent_infra/core/state/projectors/sleep.py)
  (UPSERT, first-write stamps `projected_at`, rewrite stamps
  `corrected_at`, `commit_after=False` for composition inside an
  outer transaction).
- Re-export it from
  [`projectors/__init__.py`](../../hai/src/health_agent_infra/core/state/projectors/__init__.py)
  and from the top-level
  [`core/state/projector.py`](../../hai/src/health_agent_infra/core/state/projector.py)
  re-export list.
...
[`skills/<d>-readiness/SKILL.md`](../../hai/src/health_agent_infra/skills/running-readiness/SKILL.md)
...
- The skill never mutates `forced_action` or raises confidence above
  `capped_confidence`. `hai/verification/tests/test_<d>_skill_gates.py`
...
1. **Does a hard session in this domain consume the same recovery
   ...
   [`hai/src/health_agent_infra/evals/scenarios/synthesis/`](../../hai/src/health_agent_infra/evals/scenarios/synthesis/).
...
- [`domains/<d>/intake.py`](../../hai/src/health_agent_infra/domains/nutrition/intake.py)
  for the parse/validate helpers.
- A new `hai intake <d> …` subcommand in
  [`cli.py`](../../hai/src/health_agent_infra/cli.py).
...
[`signals.py`](../../hai/src/health_agent_infra/domains/running/signals.py)
...
- **First insert stamps `projected_at`; any rewrite stamps
  ...
  tests like
  [`test_state_dual_write.py`](../../hai/verification/tests/test_state_dual_write.py)
  pin it.
...
([`guard_phase_b_mutation`](../../hai/src/health_agent_infra/core/synthesis_policy.py))
...
- `hai/verification/tests/test_<d>_schemas.py` — proposal shape == field set,
  action enum membership, `bounded=True` invariants, rejection of
  `follow_up` / `daily_plan_id` / `recommendation_id` on proposals.
- `hai/verification/tests/test_<d>_classify.py` — one test per band boundary
  per signal (the recovery/running tests are the reference — aim
  for that density).
- `hai/verification/tests/test_<d>_policy.py` — one firing + one non-firing
  test per R-rule. Verify `forced_action`, `capped_confidence`,
  `policy_decisions` contents.
- `hai/verification/tests/test_<d>_skill_gates.py` — the skill-boundary
...
- `hai/verification/tests/test_cli_propose.py` — add the domain to the
  existing parametrized coverage so `hai propose --domain <d>`
  accept/reject paths are pinned.

And eval scenarios:

- `hai/src/health_agent_infra/evals/scenarios/<d>/*.json` — at least three: a baseline
  "happy path," an R-rule firing case, and a coverage-gap case.
  [`hai/verification/tests/test_eval_scenarios.py`](../../hai/verification/tests/test_eval_scenarios.py)
...
a `hai/src/health_agent_infra/evals/scenarios/synthesis/<name>.json` scenario and an
...
10. `hai/docs/domains/README.md`'s domain list still matches
