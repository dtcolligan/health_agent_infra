...
- ``hai pull`` source resolution (v0.1.6+): explicit ``--source`` >
  ...
  ``hai/src/health_agent_infra/data/garmin/export/daily_summary_export.csv``
...
scenarios under ``hai/src/health_agent_infra/evals/scenarios/``.

If you touched a skill, ``hai eval`` is not enough. Use the opt-in harnesses
under ``hai/verification/evals/``:
...
``hai/verification/evals/skill_harness_blocker.md`` for what remains open.
