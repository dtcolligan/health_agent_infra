...
**Contributions.** (1) A capability-gated substitution effect: told the commit rule with enforcement off, capable models police themselves on every run and weak ones violate on every run, so enforcement adds nothing detectable above a capability threshold and is the whole barrier below it. We evidence it mechanistically, run by run, not with a p-value, and bound it as a confounded single-runtime case study. (2) The harness-blindness caution, demonstrated by dissolving one such finding. (3) GovernedAgentBench: a task suite, a deterministic offline scorer, and a git-pinned reference runtime that let the two levers be varied independently. The novelty is the conjunction, not any single piece: a full four-cell told-versus-enforced 2×2 on the same rule, scored on first action, per mechanism, with a released deterministic scorer. Prior work reaches the withheld-but-enforced cell but not that full crossing. The paper claims no scaling law, no injection robustness, and nothing beyond this one runtime.
...
Isolated withheld-but-enforced measurements exist: PCAS by design, Prompts Don't Protect structurally, and Mind the GAP through its enforce mode. No located prior crosses told-against-withheld with enforced-against-off for the same rule across all four cells, scored on the model's first action, per governance mechanism, under benign completion pressure, with a released deterministic offline scorer. The contribution is that conjunction, together with the harness-blindness finding (Section 6); no single element is claimed as first.
