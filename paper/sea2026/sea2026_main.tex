% =========================================================================
% SEA 2026 long-track condensation of the FINAL paper.
...
% This file is the single shared source for the SEA version; do not
% compile it directly.  Build:
%   tectonic sea2026_preprint.tex    (non-anonymous, NeurIPS 'preprint' option)
%   tectonic sea2026_anonymous.tex   (double-blind workshop submission mode)
...
\ifdefined\SEAanonymous
  ...
  \providecommand{\TargetWorkshop}{Scaling Environments for Agents (SEA)}
  \workshoptitle{\TargetWorkshop}
\else
...
% ---- macro defined here, so pointing the anonymous build at an anonymized
% ---- mirror (e.g. anonymous.4open.science) is a one-line change later.
\ifdefined\SEAanonymous
  ...
  \newcommand{\gabrepo}{\url{https://anonymous.4open.science/}}% placeholder; mirror not created yet
\else
...
\begin{figure}
\centering
\includegraphics[width=0.72\linewidth]{../figures/fig3_powered.pdf}
...
\looseness=-1 Probing prior to the experiment produced and then destroyed an apparent positive result, and the failure mode is general enough to be considered a finding. Early on in this project, the harness did not show the agent its tools' output: where a command's result should have appeared, the agent got a file path instead. When a task then needed an identifier to proceed, the agent guessed.
