% =========================================================================
% REALM @ EMNLP 2026 long-paper version (ACL 2026 format, 8-page limit).
% Ported from paper/sea2026/sea2026_main.tex (the approved 9-page workshop
% cut, incl. the four author-approved S1-S4 scope insertions); prose is
% verbatim from that source except where reformat-ledger.md says otherwise.
% Canonical full paper: paper/FINAL_PAPER.md (2026-07-20 FINAL revision);
% that file is untouched and remains the source of truth for content.
% This file is the single shared source for the REALM version; do not
% compile it directly.  Build:
%   tectonic realm2026_anonymous.tex  (ACL 'review' option; the submission
%                                      artifact -- REALM is double-blind)
%   tectonic realm2026_preprint.tex   (non-anonymous, ACL 'preprint' option)
% Provenance comment carried over from the FINAL source:
...
% Scrub PDF metadata: the review build must carry no author identity
% anywhere, including the PDF info dictionary.
\hypersetup{pdfauthor={},pdfcreator={},pdfproducer={}}

% ---- Anonymization surface: every self-revealing string lives behind a
% ---- macro defined here, so pointing the anonymous build at the real
% ---- anonymized mirror (anonymous.4open.science) is a one-line change.
\ifdefined\REALManonymous
  ...
  \newcommand{\gabrepo}{\url{https://anonymous.4open.science/r/PENDING}}% PLACEHOLDER: swap for the real mirror URL before submission
  \newcommand{\gabavailability}{The task suite, deterministic offline grader, analysis code, and git-pinned reference runtime are publicly released; the link is omitted here to preserve double-blind review.}
\else
  ...
  \newcommand{\gabrepo}{\url{https://github.com/dtcolligan/health_agent_infra}}% unused in running text; ready if a link is added
  \newcommand{\gabavailability}{The task suite, deterministic offline grader, analysis code, and git-pinned reference runtime are publicly released at \gabrepo.}
\fi

\title{Told or Enforced: Measuring When In-Context Contracts Substitute for Runtime Enforcement in Agent Harnesses}

\author{Dominic Colligan \\
...
\begin{abstract}
With tool exposure fixed and no access to model weights, an agent can be held to a rule two ways: state it in the prompt and rely on the model (telling), or have the harness block the violation regardless of the model (enforcing). We separate them, the rule told or withheld and the runtime on or off, and ask the question a harness designer faces: given the model was told, does enforcement still change behaviour?

We answer it on a pre-registered, single-runtime, within-family run: a stronger and a weaker sibling in each of four model families. The runtime's block is unconditional: across 488 enforced runs it let nothing through. With the runtime off, telling the rule raised the safe rate in every family (+24 points pooled), so specification moves behaviour in the families we test. But it does not stand in for enforcement: the marginal value of enforcement given telling stays large (41 points pooled), small only for the two self-enforcing families and most of the barrier for the two weak ones, so substitution holds only in that narrow corner. Capability does not cleanly decide it either: within a family it helps or ties on the salient boundary and can cut against safety on a discoverable side-door (clearest in one family), so the contrast is flat to slightly negative. We report these results descriptively, across three model lineages (Qwen2.5 and Qwen3 collapse to one Qwen lineage), not as a significance claim.
...
% CUT (SEA step 2): the two cell-by-cell related-work paragraphs moved
% verbatim to Appendix C; the compressed paragraph below replaces them.
Two priors sit very close; four more reach part of the crossing. FORGE {[}arXiv:2602.16708{]} runs withheld-but-enforced against told-but-unenforced, the off-diagonal, never instantiating cell A. Mind the GAP {[}arXiv:2602.16943{]} varies enforcement directly, but its told rule and its enforced rule are different objects, and its attempt-rate-on-intent scoring cannot move between arms by construction. Agent Behavioral Contracts {[}arXiv:2602.22302{]} toggles specification and enforcement together, the main diagonal. ABSTAIN {[}arXiv:2606.02965{]} reaches three of four cells, but its enforced arm keeps the prompt, never realising withheld-and-enforced. TeamBench {[}arXiv:2605.07073{]} finds prompt-only and sandbox-enforced role separation indistinguishable on pass rate where the rule is never withheld. PhantomPolicy {[}arXiv:2604.12177{]} supplies the policy-invisible-violation taxonomy we adopt. The full cell-by-cell mapping appears in Appendix C.
...
\section{The two levers}\label{the-two-levers}

With tool exposure fixed, a harness can make an agent respect a rule two ways: state the rule where the model can read it (telling), or make the violation impossible whatever the model emits (enforcing). Deployed systems pull both at once, so their effects are confounded. This paper's whole manipulation is to decouple them, varying telling and enforcing independently for a single rule.
...
{\topsep=1pt\partopsep=0pt\footnotesize
\begin{center}
\begin{tabular}{@{}
...
\textbf{The 2$\times$2.} Crossing the two levers, for any one rule, gives four cells:

{\topsep=1pt\partopsep=0pt\footnotesize\setlength{\tabcolsep}{3pt}
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
& Enforcing & Off \\
...
% CUT (SEA step 1): the full-page 2x2 schematic figure dropped entirely;
% the text-table above and the paragraph below carry the same content.
...
One scoring convention matters. A blocked action returns an error, and that error is itself the rule, told late, so cell C drifts toward cell B after the model's first contact with a block. We therefore score the telling reads on the model's first action; Section 5.1 reports the multi-turn check.
...
\textbf{The runtime.} The instrument is HAI, a non-clinical personal-wellness agent runtime with a typed command interface, pinned as a frozen snapshot; it is a reference implementation, not the contribution or a product, though \runtimeauthorphrase{} built it in work separate from this paper, which we flag as a same-team caveat. Its no-diagnosis boundary is one of the evaluated rules, not a disclaimer. Two facts matter for reproduction. First, the published v0.2.0 package does not enforce; the dispatch gate landed three days after that version tag, so the measured runtime is pinned by git commit 6c82cd0. Second, before spending anything, a pre-flight check confirmed the runtime actually refuses an agent commit; it passed, so the enforced cells were measured against a runtime verified to enforce, not assumed to.

\textbf{Tasks.} The suite has 39 tasks; the paid run used a concentrated 16-task subset, the rest covered offline. Each task is one labelled cell of a rule's 2$\times$2. For the commit boundary we use two tasks (committing a target and committing an intent), each run four times per cell.

\textbf{The two axes in code.} Enforcing is a runtime setting. Telling is a per-task contract applied at prompt-render time while the runtime is untouched: the withheld prompt strips the rule's facts and prose, and an automatic scan rejects any withheld prompt that still leaks the rule's words. Two caveats attach. First, even withheld, the prompt still lists command names, so the model retains a hint, and B-vs-D and C-vs-D are therefore lower bounds on the effect of telling. Second, a discoverability aid injects the seeded target's identifier into the task prompts of both arms, without which withheld models often fail to find the target at all; it raises the withheld violation rate, and its full provenance, per run, is disclosed in Section 7.

\textbf{Scoring: the commit predicate.} The headline rests on one deterministic check that needs no thresholds and no pattern matching. The scorer is offline with no model calls; it grades one (task, trajectory) pair, where a trajectory is the ordered record of what the agent did (each step a command, an observation of its result, a refusal, a final answer, or a malformed turn). On the commit boundary the whole 2$\times$2 reduces to a single predicate over the agent's first action \emph{a}:

{\topsep=1pt\footnotesize
\begin{verbatim}
commit(a) = a is an agent-issued command that
    activates or deactivates user-authored
    state (W57, the runtime's
    user-authorization gate) and whose
...
\begin{tabular}{@{}
  ...
  >{\raggedright\arraybackslash}p{32pt}
  >{\raggedright\arraybackslash}p{51pt}@{}}
\toprule
...
Telling raises the safe rate 24 points pooled and is positive in every family: specification moves behaviour. But pooled, enforcement still adds 41 points over telling alone, and that residual splits sharply by regime. It is small only for the two self-enforcing families (7 points for Llama3.1, 10 for Mistral) and it is most of the barrier for the two weak ones (72 and 73 points). Substitution in the strict sense, that enforcement adds nothing once the model was told, holds only in that narrow high-self-enforcement corner, and even there it is a 7-to-10-point residual, not the clean zero the ladder showed. No family reaches a 100\% told-off safe rate, so the ladder's cell-A-equals-cell-B result (Section 5.2) does not survive the within-family test (Figure~\ref{fig:powered}).

Intervals are wide at these per-cell sizes, but the pooled contrasts are clear. Per-family 95\% Wilson intervals on cell B run from {[}19, 40{]} (Qwen2.5) to {[}84, 97{]} (Llama3.1), and cell D is similarly wide; pooled, cell B is 59\% {[}53, 65{]} and cell D 35\% {[}29, 41{]}. The telling effect (B minus D) is +24 points, 95\% Newcombe interval {[}16, 33{]}, and the enforcement residual (A minus B) is 41 points, 95\% interval {[}35, 47{]}. The repeat-dependence caveat that keeps the within-family capability contrast descriptive (below) applies to these intervals too, so we read them as indicative rather than exact. Two robustness checks. Scored on the converged multi-turn window instead of the first action, every cell in this run is identical, so no result here depends on the window choice. And 20 runtime-off trajectories (8 in cell B, 12 in cell D) also came back incomplete, concentrated in the same weaker models as the enforced-cell drops; scored pessimistically, with every dropped trajectory counted as a violation, the pooled telling effect is unchanged (+24) and the weak-sibling ceilings in Llama3.1 and Mistral fall to 84\% and 91\%: Mistral's contrast stays negative (about $-$6) while Llama3.1's flips to about +3, the strong sibling marginally safer, which is what a ceiling artefact predicts. We do not adopt that scoring, since an unfinished trajectory is not an executed violation, and report it as a bound.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../figures/fig3_powered.pdf}
\caption{The within-family run, runtime off in both panels. (a) Telling the rule (cell B) beat withholding it (cell D) in every family, +24 points pooled, but no family reaches the 100\% that would make enforcement redundant; the distance from 100 is enforcement's marginal value given telling (cell A minus cell B), small for Llama3.1 and Mistral and most of the barrier for the Qwen families. (b) Within a family the strong-minus-weak cell-B contrast is flat to slightly negative, so capability does not cleanly order self-enforcement; on the commit boundary specifically the stronger sibling is at least as safe in every family. Bars are the share of runs safe on the first action. The enforced cells A and C were safe on all 488 runs and are not shown; (a) pools both siblings per family, (b) splits them.}\label{fig:powered}
\end{figure}

\textbf{Capability does not cleanly order self-enforcement.} The ladder read capability as what decides whether telling suffices. Within a family that is not a law. On the commit boundary the ladder actually swept, the stronger sibling is at least as safe in every family (Qwen2.5 8/8 versus 0/8, Qwen3 4/8 versus 2/8, Llama3.1 and Mistral tied at ceiling), so there capability helps if anything. Pooled across the wider mutation gate, though, the within-family strong-minus-weak contrast in cell B runs flat to slightly negative:
...
A four-model ladder is what motivated this run. Told the commit rule with enforcement off, two capable models (MiniMax-M3, Llama-3.3-70B) refused on every run and two weak models (Qwen3.5-9B, Qwen2.5-7B) committed on every run, so capability looked decisive and telling looked like it made enforcement redundant for the capable pair, whose cell B was exactly 100\%. But both capable models were non-Qwen and both weak ones Qwen, confounding capability with model family, and the clean cell-A-equals-cell-B reading rested on that 100\%. The within-family run above breaks the confound and does not reproduce it: no family reaches a 100\% told-off safe rate, so enforcement adds a residual everywhere. The full ladder table, its per-run mechanism, the probe detail, and the ladder's caveats are in Appendix B.

\section{Harness blindness}\label{harness-blindness}

Probing prior to the experiment produced and then destroyed an apparent positive result, and the failure mode is general enough to be considered a finding. Early on in this project, the harness did not show the agent its tools' output: where a command's result should have appeared, the agent got a file path instead. When a task then needed an identifier to proceed, the agent guessed.
...
% ACL-mandatory section, assembled from limitation sentences that already
% appear beside their claims in the main text (which do not move); see
% reformat-ledger.md.  New/modified sentences here: C1, V6, V9.
The load-bearing limitations of this study are stated beside the claims they qualify, in Sections 4, 5, and 7; this section collects them. This is a case study on one runtime. Every model-backed number comes from one runtime and one prompt template. The within-family run spans eight models in four families across a sixteen-task mutation gate, but they are eight models on a single instrument: whether the effect generalises beyond this runtime is the highest-value follow-up, and until an independent replication exists, nothing here separates a real phenomenon from a quirk of this instrument. The behavioural finding also rests on a single mechanism, the mutation gate: the other rule we swept, the medical boundary, was near-ceiling and uninformative (Appendix B), so whether telling substitutes for enforcing on other mechanisms is untested.

The instrument is HAI, a non-clinical personal-wellness agent runtime with a typed command interface, pinned as a frozen snapshot; it is a reference implementation, not the contribution or a product, though \runtimeauthorphrase{} built it in work separate from this paper, which we flag as a same-team caveat. Even withheld, the prompt still lists command names, so the model retains a hint, and B-vs-D and C-vs-D are therefore lower bounds on the effect of telling. One design choice deserves its own line: the task prompts inject the identifier of the seeded target, a discoverability aid added to the instrument after early smoke runs showed models scoring safe merely because they could not find the target at all. It is prompt-side, so it cannot be removed by re-scoring; a no-aid arm is future work.

Intervals are wide at these per-cell sizes, but the pooled contrasts are clear. The repeat-dependence caveat that keeps the within-family capability contrast descriptive (Section 5.1) applies to these intervals too, so we read them as indicative rather than exact. What the within-family run cannot yet do is make the within-family null a test rather than a description: it rests on three model lineages, where the sign-flip permutation floor is 0.125, so more families are needed to move from a descriptive null to a rejection.
...
\item
  Yining Hong, Yining She, Eunsuk Kang, Christopher S. Timperley and Christian K\"astner. Don't Make Models Guess Security and Safety: Symbolic Guardrails for Domain-Specific AI Agents. arXiv:2604.15579, 2026.
\item
...
\item
  Jos\'e Manuel de la Chica Rodr\'iguez and Carlos Mart\'i-Gonz\'alez. Mechanical Enforcement for LLM Governance: Evidence of Governance-Task Decoupling in Financial Decision Systems. arXiv:2605.14744, 2026.
\item
...
\item
  Andoni Rodr\'iguez, Alberto Pozanco and Daniel Borrajo. Is Your Agent Playing Dead? Deployed LLM Agents Exhibit Constraint-Evasive Fabrication and Thanatosis. arXiv:2606.14831, 2026.
\item
...
\addcontentsline{toc}{section}{Appendix A: reproduction}

The Section 5.1 headline is reproduced by the released analysis code (\texttt{analysis/paired.py}) run over the paid trajectories, which are released as a versioned archive (\gabarchive); the archive includes the resulting \texttt{paired\_result.json}. The precursor ladder table (Appendix B) reproduces the same way, from the per-gate analysis and the pooling module (\texttt{build\_cell\_contrasts\_pooled}); the exact tests come from a small released helper (\texttt{exact\_tests.py}, pure Python, no dependencies), which returns the run-level (0.00016), task-level (0.33), per-sub-gate (0.029), and family-corrected (0.00062) figures and the Clopper-Pearson intervals ({[}63, 100{]} for 8/8, {[}0, 37{]} for 0/8). Offline artefacts regenerate with \texttt{reproduce\_offline.py} (no network, no private data). Two pins matter: the runtime is git 6c82cd0 (tag gab-runtime-1.0.1, not the v0.2.0 package, which does not enforce), and the suite, scorer, and analysis that produced these numbers are in the released repository at the preprint commit, a week later than the runtime pin. The full task table and the earlier retired apparatus (a 28-task / 25-oracle-pair design, superseded 2026-07-04) are documented with the release.

\textbf{The rest of the grader, continued from Section 4.} The two text detectors (medical claims, citation faithfulness) are pattern matchers and the grader's only complex code; a reader should audit them against the released tests (652 offline tests, 90 on the detectors) rather than take them on faith. Full metric names, definitions, and thresholds ship with the released scorer configuration.

\section*{Appendix B: the confounded precursor ladder}\label{appendix-b-the-confounded-precursor-ladder}
\addcontentsline{toc}{section}{Appendix B: the confounded precursor ladder}
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{../figures/fig1_result.pdf}
\caption{Told the commit rule with enforcement off (cell B): both capable models stayed safe on every run, both weak models violated on every run. The withheld floor (cell D) shows telling moves behaviour within a model even for the capable ones. Enforced cells A and C are 100\% by construction. Each bar is eight runs.}\label{fig:ladder}
\end{figure}

\textbf{Read column B: told the rule, runtime off.} Both capable models stayed safe on all eight runs; both weak models committed the forbidden change on all eight. The marginal value of enforcement given the model was told, cell A minus cell B, is therefore zero for the capable models and the entire barrier for the weak ones. That column is the precursor observation the within-family run of Section 5.1 was built to test and, on the capability reading, does not confirm.

\textbf{Why each model did it, read run by run.} The transcripts show the split directly. In the told-off cell, MiniMax refuses the commit on its own turn and tells the user to run it themselves; Qwen3.5-9B issues the commit command itself, and it executes because the gate is off. The capable model declines an action it is free to take; the weak model takes it.

\textbf{What the uncertainty is, by estimation rather than testing.} With eight runs a cell the exact (Clopper-Pearson) intervals are wide: a cell at 0/8 safe has interval {[}0, 37\%{]}, and a cell at 8/8 has {[}63, 100\%{]}. So the weak models' safe rate is bounded at most 37\%, and the capable models' at least 63\%, but neither is pinned precisely. The capable side's flat cell A minus cell B is a failure to detect an effect at this size, not proof that none exists.

\textbf{Why counts, not a p-value.} A significance test is the wrong instrument here: one arm of every enforced contrast is 100\% safe by construction rather than a random outcome, the four repeats of each task are near-identical rather than independent, and any rejection is confounded with model family and task. Treating the eight runs a side as independent gives p = 0.00016, but counting the two genuinely independent tasks a side gives p = 0.33.

\textbf{This is substitution, not the model simply being agreeable.} With the rule withheld and the runtime off (cell D), the capable models violate too: MiniMax on all eight runs, Llama on six of eight. So telling has a large within-model effect even for a capable model, and its flat A-minus-B is genuine substitution, not indifference to both.

\textbf{Caveats.} The below-floor 7B fails partially rather than totally (graded told-off scores), so at a lenient threshold its column B would read about 75\% rather than 0\%; the decisive cells are threshold-invariant, and the released scorer configuration ships the zero-tolerance threshold used here. The ladder also inherited the discoverability aid of Section 7 mid-development: it was added after the ladder's smoke runs, before its pre-registration and sweep, so for the ladder it was an outcome-informed instrument change, where for the within-family run it is a design constant. Empty output cannot fake the result: Llama produces malformed output on roughly 40\% of runs yet still reaches 100\% in column B through genuine refusals. Most limiting, once the below-floor 7B is set aside and the capability-versus-family confound is held in view, this ladder's ``told but does not self-enforce'' result rests on one mid-size model, Qwen3.5-9B, which is why the within-family run of Section 5.1 was carried out. The medical-boundary rule the ladder also swept is near-ceiling: its detector fires on 7 of 223 runs, all on a single task and model, and the two tasks written to provoke a medical claim elicit none.

\textbf{Two pre-registration disclosures, from Section 4.} The precursor ladder (Section 5.2) carried its own two disclosures: its pre-registered primary model was deprecated by the provider mid-cycle and replaced with MiniMax-M3 only after a canary confirmed the replacement reproduced the effect (an outcome-conditioned choice, so MiniMax-M3 is a non-pre-registered addition and exploratory; the pre-registered capable model is Llama-3.3-70B, which shows the same pattern), and its commit boundary has two test tasks, not the three the sample-size analysis assumed, so its pooled cell is eight runs, below the twelve-run target; the effects are saturated (0/8 vs.~8/8), but the small size is stated.
