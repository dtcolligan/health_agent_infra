...
``hai/reporting/plans/historical/phase_2_5_retrieval_gate.md``) compressed Phase 5
to macros-only:
...
Phase 3. v0.1.8 partially closes it with ``hai/verification/evals/skill_harness/``
for recovery + running readiness outputs and
``hai/verification/evals/synthesis_harness/`` for synthesis-skill output fixtures.
...
- Evaluation framework at ``hai/verification/evals/`` covering domain
