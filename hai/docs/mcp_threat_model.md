...
- `hai/docs/privacy.md` — data-handling posture.
...
- **Mitigation (Wave 3 contract).** MCP server is shipped inside
  ...
  in `hai/reporting/AUDIT.md` covers MCP-surface changes.
...
- **Gap.** Wave 3 SBOM publication. Destination: v0.3 ship-time
  freshness sweep adds a `hai/docs/sbom.md` row.
...
- **Gap.** Bandit run on the MCP-surface module. Destination:
  v0.3 ship gate adds `uvx bandit -ll -r hai/src/health_agent_infra/mcp/`
...
| SBOM publication | v0.3 (MCP04 mitigation) | New `hai/docs/sbom.md`. |
| Bandit on `hai/src/health_agent_infra/mcp/` | v0.3 (MCP05 mitigation) | New ship gate. |
...
- **`hai/reporting/plans/post_v0_1_13/cycle_proposals/CP-DO-NOT-DO-ADDITIONS.md`** —
  the cycle proposal that landed the additions.
- **`hai/reporting/plans/strategic_plan_v1.md`** §10 + post-v0.1.18
