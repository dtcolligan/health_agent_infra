...
- ``hai/verification/tests/test_<d>_schemas.py`` — schema validation.
- ``hai/verification/tests/test_<d>_classify.py`` — one test per band
  boundary per signal.
- ``hai/verification/tests/test_<d>_policy.py`` — one test per R-rule
  firing + non-firing.
- ``hai/verification/tests/test_<d>_skill_gates.py`` — skill-boundary
  contract (the skill does NOT change forced actions, DOES compose
  rationale, etc).
- ``hai/src/health_agent_infra/evals/scenarios/<d>/*.json`` — ≥ 3 scenarios (baseline,
  R-rule firing, coverage gap).

``hai/verification/tests/test_eval_scenarios.py`` has a parametrized test
...
``hai/src/health_agent_infra/evals/scenarios/synthesis/`` and an entry in the
...
- [ ] ``hai/docs/state_model_v1.md`` updated with the new table
- [ ] ``hai/docs/architecture.md`` pipeline ASCII updated
- [ ] ``hai/docs/x_rules.md`` updated if an X-rule changed
