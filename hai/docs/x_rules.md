...
``hai/src/health_agent_infra/core/synthesis_policy.py`` with one
evaluator per rule.
...
``hai/src/health_agent_infra/core/synthesis_policy.py``. Adding a rule
...
``hai/src/health_agent_infra/evals/scenarios/synthesis/``.
``hai/verification/tests/test_eval_scenarios.py ::
test_synthesis_scenarios_cover_key_x_rules`` is the floor-check for that
scenario set.

X4 and X5 have dedicated unit/integration tests in
``hai/verification/tests/test_synthesis_x3_x4_x5_strength.py`` because they need
...
``hai/verification/tests/test_synthesis_policy.py``. Moving X4/X5/X6b into
