...
Two assertions:
  ...
  2. Every `hai <subcommand>` reference in `hai/docs/*.md` also
     resolves.
...
SKILLS_DIR = HAI_ROOT / "src" / "health_agent_infra" / "skills"
DOCS_DIR = HAI_ROOT / "docs"
...
def test_reporting_docs_hai_refs_resolve() -> None:
    """Every `hai <subcommand>` reference under `hai/docs/` must
    ...
    # Soft-fail for now: hai/docs may include prose examples that
    ...
    if errors:
        pytest.skip(
            "hai/docs contains references to commands not in the "
