...
def test_schema_migrations_has_one_row_per_applied_migration(tmp_path: Path):
    ...
    assert [tuple(r) for r in rows] == [
        ...
        (25, "025_target_macros_extension.sql"),
        (26, "026_body_comp.sql"),
        (27, "027_evidence_card_daily.sql"),
        (28, "028_evidence_card_weekly.sql"),
    ]
...
def test_current_schema_version_matches_head_after_init(tmp_path: Path):
    ...
    conn = open_connection(db_path)
    try:
        assert current_schema_version(conn) == 28
    finally:
        conn.close()
...
def test_cli_state_migrate_on_head_db_reports_empty_applied(tmp_path: Path, capsys):
    ...
    assert payload["schema_version_before"] == 28
    assert payload["schema_version_after"] == 28
