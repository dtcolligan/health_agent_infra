...
That rejection should fail closed with error code `ungrounded_evidence_ref` and should not mutate the successful recommendation artifact. The payload now also needs `resolution_window_artifact_path` plus `policy_basis`, matching the current recommendation contract. If you still need the legacy compatibility lane for an older proof surface, `health_model.agent_recommendation_cli` remains available, but it is no longer the taught default for this public demo path.
